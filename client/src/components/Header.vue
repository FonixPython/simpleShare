<template>
  <div
    id="header"
    class="absolute top-0 left-0 w-full h-[100px] mobile:h-[80px] border-b border-b-[length:1px] border-b-gradient-to-r from-blue-400 via-green-400 to-blue-400 animate-slide-down backdrop-blur-md bg-black/20"
  >
    <Logo />
    <div
    ...
    >
      ...
      <div
        v-if="isAuthenticated && quotaInfo.text"
        id="quota-container"
        class="block mobile:block"
      >
        <div class="flex flex-col items-center gap-2">
          <span
            id="quota-text"
            class="text-sm whitespace-nowrap mobile:text-xs"
            >{{ quotaInfo.text }}</span
          >
          <div class="flex items-center gap-2">
            ...
            <span
              id="percentage-text"
              class="text-sm mobile:text-xs font-medium transition-all duration-300"
              >{{ quotaInfo.percentage }}%</span
            >
          </div>
        </div>
      </div>
...
export default {
  ...
  emits: ["show-login", "show-upload", "show-share-link", "toggle-hamburger"],
  methods: {

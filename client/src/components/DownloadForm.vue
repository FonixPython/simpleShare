    ...
    <form @submit.prevent="handleSubmit">
      ...
      >
        <span
          ...
          class="material-icons-outlined animate-spin-slow"
...
export default {
  ...
  methods: {
    ...
    handlePaste(event, index) {
      ...
      for (
        let i = index;
        i < this.inputs.length && charIndex < pastedData.length;
        i++
      ) {
          ...
          if (result && result.exists) {
            ...
            if (result.type === "group") {
              this.fileData = {
                ...
                total_size: result.files.reduce(
                  (total, file) => total + file.file_size_in_bytes,
                  0,
                ),
    ...
    async handleSubmit() {
      ...
      this.inputs.forEach((input, index) => {
        setTimeout(() => {
          ...
          setTimeout(() => {
            this.$refs.codeInputs[index].classList.remove(

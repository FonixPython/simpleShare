...
export function useFiles() {
  ...
  const updateQuotaDisplay = async (token) => {
    try {
      ...
      if (data.total !== 0) {
        quotaInfo.value.text =
          formatBytes(data.used) + " of " + formatBytes(data.total);
      ...
      files.value = result.map((item) => {
        if (item.type === "group") {
          return {
            ...
            formattedDate:
              new Date(item.create_date).toLocaleDateString("hu-HU", {
                year: "2-digit",
                month: "2-digit",
                day: "2-digit",
              }) +
          ...
          return {
            ...
            formattedDate:
              new Date(item.upload_date).toLocaleDateString("hu-HU", {
                year: "2-digit",
                month: "2-digit",
                day: "2-digit",
              }) +
  ...
  const deleteFile = async (code, token, deleteSubItems = false) => {
    try {
      const url = deleteSubItems
        ? `/delete/${code}?deleteSubItems=true`
        : `/delete/${code}`;
      ...
      xhr.addEventListener("load", function () {
        ...
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          if (xhr.status === 413) {
            ...
            errorMessage =
              "Invalid file format or request. Please check your files and try again.";

                    ...
                    <div class="flex items-center justify-center gap-2">
                      ...
                      >
                        <span class="material-icons text-xs">{{
                          user.is_admin ? "admin_panel_settings" : "person_add"
                        }}</span>
                      </button>
                      <button
                        @click="
                          handlePasswordChange(user.user_id, user.username)
                        "
                      ...
                      <button
                        @click="
                          handleUsernameChange(user.user_id, user.username)
                        "
      ...
      >
        ...
        <div class="mb-6">
          <p class="text-gray-300 mb-3 text-lg">
            ...
            {{ changeRoleDialog.user?.is_admin ? "demote" : "promote" }}
            <span class="text-primary-button font-semibold">{{
              changeRoleDialog.user?.username
            }}</span>
            {{
              changeRoleDialog.user?.is_admin
                ? "from admin to user"
          ...
          <p class="text-gray-400 text-sm leading-relaxed">
            {{
              changeRoleDialog.user?.is_admin
                ? "This will remove the user's administrative privileges and they will no longer be able to manage users, files, or system settings."
      ...
      >
        ...
        <div class="mb-6">
          <p class="text-gray-300 text-lg">
            Change password for
            <span class="text-primary-button font-semibold">{{
              changePasswordDialog.user?.username
            }}</span>
          </p>
        </div>
    ...
    >
      ...
      >
        <div class="flex items-center justify-between mb-6">
          ...
          <div
            class="w-12 h-12 rounded-full bg-secondary-button/20 flex items-center justify-center"
          >
            <span class="material-icons text-xl text-secondary-button"
              >edit</span
            >
          </div>
        </div>
        ...
        <div class="mb-6">
          <p class="text-gray-300 text-lg">
            Change username for
            <span class="text-primary-button font-semibold">{{
              changeUsernameDialog.user?.username
            }}</span>
          </p>
        </div>
      ...
      >
        ...
        <div class="mb-6">
          <p class="text-gray-300 mb-2 text-lg">
            Change quota for
            <span class="text-primary-button font-semibold">{{
              changeQuotaDialog.user?.username
            }}</span>
          </p>
          <div class="bg-gray-800/30 rounded-xl p-4 border border-gray-700/50">
            <p class="text-gray-400 text-sm">Current quota:</p>
            <p class="text-white font-semibold">
              {{ changeQuotaDialog.user?.quotaFormatted }}
            </p>
          </div>
          <p class="text-gray-400 text-sm mt-3 leading-relaxed">
            This will affect the user's upload limits. If quota is exceeded, the
            user cannot upload new files. Please verify the new quota carefully!
          </p>
        </div>
        ...
        <div class="mb-6">
          <p class="text-gray-300 mb-3 text-lg">
            ...
            <span class="text-red-400 font-semibold">{{
              deleteUserDialog.user?.username
            }}</span
            >?
          </p>
          <div class="bg-red-500/10 rounded-xl p-4 border border-red-500/20">
            ...
            <p class="text-gray-300 text-sm leading-relaxed">
              ...
              and all their files will be removed.
            </p>
          </div>
        </div>
...
export default {
  ...
  setup(props) {
    ...
    const handleDeleteUser = async (user) => {
      if (
        confirm(
          `Are you sure you want to delete user "${user.username}"? This will also delete all their files.`,
        )
      ) {
    ...
    const handleDeleteFile = async (file) => {
      if (
        confirm(`Are you sure you want to delete file "${file.originalname}"?`)
      ) {
    ...
    const handleSaveUser = async () => {
      const { user, newUsername, newPassword, newQuota, isAdmin } =
        editDialog.value;
      ...
      if (newPassword) {
        hasChanges = true;
        promises.push(
          changeUserPassword(props.token, user.user_id, newPassword),
        );
      }
      ...
      if (isAdmin !== user.is_admin) {
        hasChanges = true;
        promises.push(
          changeUserAdminStatus(props.token, user.user_id, isAdmin),
        );
      }
      ...
      try {
        ...
        if (failed.length === 0) {
          ...
          showNotification(
            "Some changes failed: " + failed.map((f) => f.error).join(", "),
            "error",
          );
        }
      } catch (error) {
    ...
    const handleQuotaChange = (userId, username, currentQuota) => {
      ...
      openChangeQuotaDialog(
        fullUser || { user_id: userId, username, quota: currentQuota },
      );
    };
    ...
    const handleChangeRole = async () => {
      ...
      const result = await changeUserAdminStatus(
        props.token,
        user.user_id,
        newAdminStatus,
      );
      if (result.success) {
        showNotification(
          `User ${newAdminStatus ? "promoted to" : "demoted from"} admin successfully!`,
          "ok",
        );
    ...
    const handleChangeUsername = async () => {
      ...
      if (!adminPassword || !newUsername) {
        showNotification(
          "Admin password and new username are required",
          "error",
        );
        return;
      }
    ...
    const handleChangeQuota = async () => {
      const { user, adminPassword, newQuotaValue, newQuotaUnit } =
        changeQuotaDialog.value;
      ...
      const result = await changeUserQuota(
        props.token,
        user.user_id,
        newQuotaInBytes,
      );

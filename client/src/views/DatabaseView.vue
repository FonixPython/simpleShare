      ...
      <div class="mb-6 flex flex-col sm:flex-row gap-4">
        <div class="flex-1">
          ...
          >
            ...
            <option v-for="table in databaseTables" :key="table" :value="table">
              {{ table }}
            </option>
          </select>
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            <thead class="bg-black/30">
              <tr>
                <th
                  class="px-4 py-3 text-left text-sm font-medium text-gray-300 w-12"
                >
                  #
                </th>
                <th
              ...
              >
                ...
                <td
                  ...
                  :class="[
                    ...
                    selectedCell === `${getRowId(row)}-${column}`
                      ? 'bg-yellow-500/20'
                      : '',
                  ]"
                ...
                >
                  ...
                  <span
                    v-else
                    @dblclick="startEditingCell(getRowId(row), column)"
  ...
  setup(props) {
    ...
    const deleteSelectedRow = () => {
      ...
      if (confirm("Are you sure you want to delete this row?")) {
        const index = tableData.value.findIndex(
          (row) => getRowId(row) === selectedRow.value,
        );
        if (index > -1) {
          ...
          hasChanges.value = true;
          showNotification(
            "Row marked for deletion. Click Save to persist changes.",
            "info",
          );
        }
      }
    ...
    const clearSelectedCell = () => {
      ...
      if (row) {
        ...
        hasChanges.value = true;
        showNotification(
          "Cell cleared. Click Save to persist changes.",
          "info",
        );
      }
    };

              ...
              <div>
                ...
                <div class="space-y-3">
                  <div>
                    ...
                    <div
                      class="w-full bg-gray-700 rounded-full h-4 overflow-hidden"
                    >
                      <div
...
export default {
  ...
  setup(props) {
    ...
    const calculateNewLimit = () => {
      ...
      return (
        newStorageLimitValue.value *
        (multipliers[newStorageLimitUnit.value] || 1)
      );
    };
    ...
    const parseCurrentLimit = (limitInBytes) => {
      ...
      for (const unit of units) {
        if (
          limitInBytes >= multipliers[unit] &&
          limitInBytes % multipliers[unit] === 0
        ) {
      ...
      newStorageLimitValue.value =
        Math.round((limitInBytes / (1024 * 1024)) * 100) / 100;
    ...
    const usagePercentage = computed(() => {
      if (currentLimit.value === 0) return 0;
      return Math.round(
        (globalStorage.value.totalStorage / currentLimit.value) * 100,
      );
    });

...
export const deleteItem = async (req: Request, res: Response) => {
  ...
  if (user_permission.level === "user") {
    delete_result = await uploadActions.deleteItem(code, deleteSubItems, user_permission.user_id)
  }
...
export const deleteGroups = async (req: Request, res: Response) => {
  ...
  if (user_permission.level === "user") {
    delete_result = await uploadActions.deleteItem(code, true, user_permission.user_id)
  }
  switch (delete_result) {
    case (0): return res.sendStatus(200); break;
    case (1): return res.sendStatus(404); break;
